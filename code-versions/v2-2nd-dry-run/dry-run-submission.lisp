...
(defun test()
  (setq sdr-start (get-internal-run-time))
  ...
  (setq sdr-fin (get-internal-run-time))
  (format t "total time for ~s rounds: ~s ~&" 1 (float (/ (- sdr-fin sdr-start) 1000000)))
  
  (format t "avg seconds per guess : ~s ~&" (float (/ (float (/ (- sdr-fin sdr-start) 1000000)) 1))))
