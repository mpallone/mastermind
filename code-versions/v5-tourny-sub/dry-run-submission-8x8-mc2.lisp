...
(defvar *superconstant*)
;(setf *superconstant* 0)

;(setq *high-punish-score* 4)
...
(defun mm-gentest (&optional (colors *colors*) (code-length *code-length*)
  ...
  ;"Super 1337 guesser"
	...
	(if (and (< *mm-code-length* 5) (>= *mm-code-length* 4)) (setf *superconstant* 1000))
	;(print *superconstant*)
	;; set the punishable score to be 25% of the highest possible score
	(setf maximum-score (basic-heuristic code-length 0))
	(setf *high-punish-score* (* .14 maximum-score))
...
;(format t "Guessing ~s...~%" (number-to-guess *cfg*))
	...
	(let ((guess *cfg*) (red 0) (white 0) (time (get-universal-time)))
		(loop while (not (eq red *mm-code-length*)) do
			...
			(setf score (basic-heuristic red white))
...
(defun mm-use-score (&optional (colors *colors*) (code-length *code-length*)
			     (generator #'mm-gen-random)
			     &aux reds whites)
	(setf *mm-colors* colors)
...
;(format t "Guessing ~s...~%" (number-to-guess *cfg*))
	...
	(let ((guess *cfg*) (red 0) (white 0) (time (get-universal-time)))
		(loop while (not (eq red *mm-code-length*)) do
			...
			;(print 'printing_guess)
			;(print 'getting_score)
			(multiple-value-setq (red white) (mm-score (number-to-guess guess) '(red blue green violet)  '(red yellow blue green indigo violet)   ))
			;(format t "Guessing ~s got reds: ~d whites: ~d =~d~%" (number-to-guess guess) red white (basic-heuristic red white))
			;(print 'set_prev-guesses)
			(setf *prev-guesses* (cons (list guess (list red white)) *prev-guesses*))
			;(print 'checking_heuristic)
