 ...
 ;;(print 'swap)
  (loop while (<= 2 (length white-list)) do
	...
	(setq first-index (nth index1 white-list))
       (setq used-white-list (cons first-index used-white-list))
	...
	(setq used-white-list (cons second-index used-white-list ))
	(setq white-list (remove second-index white-list))
	;(format t "used-white-list ~s ~&" used-white-list)
	;(format t "swapping: ~s with ~s ~&" first-index second-index)
  ...
  ;; index-list
  ;(print "swapping last white")
  ;(format t "whitelist length: ~d, index-length: ~d" (length white-list) (length index-list))
...
;	    (setf (nth second-swap-index cfg-copy) color1))))  <== this need sto be random
	   ...
	   ; (format t "moving: ~s to ~s ~&" second-swap-index first-swap-index)
	
	    	(setq index-list (remove swap index-list))
	    ;; randomize the leftover index, second-swap-index
	    (setf (nth 1 (nth color2 *color-pool*)) 0)
	    ;(print *color-pool*)
	    (setf color-index (prob-random-color (sum-prob)))
	    (setf (nth 1 (nth color2 *color-pool*)) 100)
	    (setf color (car (nth color-index *color-pool*)))
	    ;(format t "replacing: ~s with ~d ~&" second-swap-index color)
	    (setf (nth second-swap-index cfg-copy) color))))

;(print "swapping last white in odd case")
  ;(format t "whitelist length: ~d, used-white-length: ~d" (length white-list) (length used-white-list))
  (if (eq 1 (length white-list))
      (if (<= 1 (length used-white-list))
	  (progn
	    (setq index1 (random (length used-white-list)))
	    (setq first-swap-index (nth index1 used-white-list))	    
	    (setq second-swap-index (nth 0 white-list))
...
;	(setf (nth 1 (nth color-pool-index *color-pool*)) 0))
 ...
 ;;(print 'assign_new)
  ...
  (loop for i in index-list do
	(setf color-index (prob-random-color (sum-prob)))
	(setf color (car (nth color-index *color-pool*)))
		;(format t "setting ~d to ~d ~%" i color)
